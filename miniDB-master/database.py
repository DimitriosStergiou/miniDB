...
class Database:
    ...
    def __init__(self, name, load=True):
        ...
        self.create_table('meta_locks',  ['table_name', 'locked','lockedS','lockedXrow','lockedSrow'], [str, bool,bool,str,str])
    ...
    def drop_table(self, table_name):
        ...
        '''
        self.load(self.savedir)
        if self.is_locked(table_name) or self.is_lockedS(table_name):
            return
    ...
    def insert(self, table_name, row, lock_load_save=True):
        ...
        if lock_load_save:
            self.unlock_table(table_name,False)
            self._update()
            self.save()
    ...
    def update(self, table_name, set_value, set_column, condition):
        ...
        '''
        self.load(self.savedir)
        if self.is_lockedS(table_name) or self.is_locked(table_name):
            return
            
        if self.is_lockedXrow(table_name,condition) or self.is_lockedSrow(table_name,condition):
            return
    ...
    def select(self, table_name, columns, condition=None, order_by=None, asc=False,\
        ...
        if self.is_lockedXrow(table_name,condition):
            return
    ...
    def lockX_tableRow(self, table_name,condition):
        '''
        Locks the specified table using the exclusive lock (X)
        ...
        if self.is_lockedXrow(table_name,condition) or self.is_lockedSrow(table_name,condition):
            return
        self.tables['meta_locks']._update_row(condition, 'lockedXrow', f'table_name=={table_name}')
        self._save_locks()
        # print(f'Locking table "{table_name}"')
    ...
    def lockS_tableRow(self, table_name,condition):
        '''
        Locks the specified table using the exclusive lock (X)
        ...
        if self.is_lockedXrow(table_name,condition):
            return
        self.tables['meta_locks']._update_row(condition, 'lockedSrow', f'table_name=={table_name}')
        self._save_locks()
        # print(f'Locking table "{table_name}"')
    
    def unlock_table(self, table_name,rows=True):
        '''
        ...
        if rows:
            self.tables['meta_locks']._update_row("", 'lockedXrow', f'table_name=={table_name}')
            self.tables['meta_locks']._update_row("", 'lockedSrow', f'table_name=={table_name}')
        self._save_locks()
    ...
    def is_locked(self, table_name):
        ...
        try:
            ...
            if res:
                print(f'Table "{table_name}" is currently locked at exclusive mode.')
            return res
    ...
    def is_lockedXrow(self, table_name,condition):
        '''
        Check whether the specified table is exclusivelly locked (X)
        ...
        try:
            res = self.select('meta_locks', ['lockedXrow'], f'table_name=={table_name}', return_object=True).lockedXrow[0]
            if res==condition:
                print(f'This row of table "{table_name}" is currently locked at exclusive mode.')
                return True
        except IndexError:
            return
  
  
    def is_lockedS(self, table_name):
        '''
        Check whether the specified table is exclusivelly locked (X)
        ...
        try:
            ...
            if res:
                print(f'Table "{table_name}" is currently locked at share mode.')
            return res
    ...
    def is_lockedSrow(self, table_name,condition):
        '''
        Check whether the specified table is exclusivelly locked (X)
        ...
        try:
            res = self.select('meta_locks', ['lockedSrow'], f'table_name=={table_name}', return_object=True).lockedSrow[0]
            if res==condition:
                print(f'This row of table "{table_name}" is currently locked at share mode.')
                return True
        except IndexError:
            return
